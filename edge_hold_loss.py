...
def weighted_saliency(input_, target, eps=1e-15):
        saliency_loss = -1.12 * target * torch.log(F.sigmoid(input_) + eps) - 1 * (1 - target) * torch.log(1 - F.sigmoid(input_) + eps)
        return torch.mean(saliency_loss)

class EdgeHoldLoss(nn.Module):
    def __init__(self, device):
        super(EdgeHoldLoss, self).__init__()
        self.laplacian_kernel = torch.tensor([[-1., -1., -1.], [-1., 8., -1.], [-1., -1., -1.]], dtype=torch.float, requires_grad=False).view(1,1,3,3).to(device)
        # !!!!begining=1


    def forward(self, y_pred, y_gt, alpha_sal):
        ...
        edge_loss = torch.mean(F.binary_cross_entropy(input=F.sigmoid(y_pred_edges), target=y_gt_edges))
